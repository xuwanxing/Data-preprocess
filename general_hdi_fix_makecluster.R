...
parallel_time <- system.time({
  parallel_res <- foreach(i=1:length(pick_genes), .combine = rbind) %dopar% estimate.sigma.fit.hdi(x,y,pick_genes[i])
})
stopCluster(cl)
registerDoSEQ()
...
filename <- args[3]
write.table(parallel_res, file=paste0(getwd(),"/",filename,"_lasso_hdi.txt"), quote=F, sep="\t",col.names = NA, row.names = TRUE)
